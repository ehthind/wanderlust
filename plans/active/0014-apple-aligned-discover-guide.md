...
## Summary
Rebuild the iPhone Discover destination guide around Apple-aligned layout rules while preserving Wanderlust's editorial feel. The guide now opens from a left swipe on the active Discover card, keeps tab chrome visible, supports right-swipe or back-button exit into Discover, uses safe-area-aware grouped content, and routes into the trip workspace through a single bottom CTA.

## Decisions
- Keep Discover as a full-screen vertical card feed on iPhone.
- Open the destination guide from a left swipe on the active card instead of the previous adjacent profile surface or a visible feed CTA.
...
- Support both explicit back navigation and swipe-out dismissal from the guide.
...
- [x] add gesture-only guide entry, guide exit gestures, guide/back accessibility identifiers, and updated UI coverage
